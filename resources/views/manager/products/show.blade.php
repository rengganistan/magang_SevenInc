    ...
    <div class="grid grid-cols-1 lg:grid-cols-3 gap-6">
        ...
        <div class="space-y-4">
            ...
            <div class="rounded-2xl border border-gray-700 bg-gray-800 p-5 flex flex-col items-center">
                ...
                @else
                    <div class="w-full max-w-xs h-48 rounded-xl bg-gray-700 flex items-center justify-center text-6xl">
                        📦
                    </div>
                @endif
            ...
            <div class="rounded-2xl border border-gray-700 bg-gray-800 p-5">
                ...
                <div class="space-y-3">
                    ...
                    <div class="pt-2 border-t border-gray-700">
                        @if($product->stok == 0)
                            <p class="text-xs text-red-400">⚠ Stok habis! Segera lakukan restok.</p>
                        @elseif($product->stok <= $product->stok_minimum)
                            <p class="text-xs text-yellow-400">⚠ Stok di bawah minimum, pertimbangkan restok.</p>
                        @else
        ...
        <div class="lg:col-span-2 space-y-4">
            ...
            <div class="rounded-2xl border border-gray-700 bg-gray-800 p-6">
                ...
                <div class="grid grid-cols-1 md:grid-cols-2 gap-4">
                    ...
                    <div>
                        ...
                        <span class="px-3 py-1 rounded-full bg-blue-600/20 text-blue-400 text-sm font-medium">
            ...
            {{-- Harga --}}
            <div class="rounded-2xl border border-gray-700 bg-gray-800 p-6">
                <h3 class="text-sm font-semibold text-gray-400 uppercase mb-4">Informasi Harga</h3>
                <div class="grid grid-cols-1 md:grid-cols-3 gap-4">

                    <div class="rounded-xl bg-gray-700/50 p-4 text-center">
                        <p class="text-xs text-gray-400 mb-1">Harga Beli</p>
                        <p class="text-white font-bold text-xl">Rp {{ number_format($product->harga_beli, 0, ',', '.') }}</p>
                    </div>

                    <div class="rounded-xl bg-green-600/10 border border-green-600/20 p-4 text-center">
                        <p class="text-xs text-gray-400 mb-1">Harga Jual</p>
                        <p class="text-green-400 font-bold text-xl">Rp {{ number_format($product->harga_jual, 0, ',', '.') }}</p>
                    </div>

                    <div class="rounded-xl bg-blue-600/10 border border-blue-600/20 p-4 text-center">
                        <p class="text-xs text-gray-400 mb-1">Margin</p>
                        @php $margin = $product->harga_jual - $product->harga_beli; @endphp
                        <p class="text-blue-400 font-bold text-xl">Rp {{ number_format($margin, 0, ',', '.') }}</p>
                    </div>
            ...
            @php
                $recentTx = $product->stockTransactions()->with('user')->latest()->take(5)->get();
            @endphp
            @if($recentTx->count())
            <div class="rounded-2xl border border-gray-700 bg-gray-800 p-6">
                <h3 class="text-sm font-semibold text-gray-400 uppercase mb-4">Riwayat Transaksi Terakhir</h3>
                <div class="space-y-2">
                    ...
                    <div class="flex items-center justify-between py-2 border-b border-gray-700 last:border-0">
                        <div class="flex items-center gap-3">
                            ...
                            @endif
                            <span class="text-gray-300 text-sm">{{ $tx->user->name ?? '-' }}</span>
                        </div>
