...
<div class="w-full px-4 py-6 sm:px-6 lg:px-8">
                ...
                <tbody class="divide-y divide-gray-700">
                    ...
                    <tr class="hover:bg-gray-700/50">
                        ...
                        <td class="px-6 py-4 text-center">
                            @if($item->selisih > 0)
                                <span class="px-3 py-1 rounded-full bg-green-600/20 text-green-400 text-sm font-semibold">+{{ $item->selisih }}</span>
                            @elseif($item->selisih < 0)
                                <span class="px-3 py-1 rounded-full bg-red-600/20 text-red-400 text-sm font-semibold">{{ $item->selisih }}</span>
                            @else
                                <span class="px-3 py-1 rounded-full bg-gray-600/30 text-gray-400 text-sm">0</span>
                            @endif
                        </td>
        ...
        <div class="flex flex-col sm:flex-row items-start sm:items-center justify-between gap-4">
            <div>
                ...
                <p class="text-sm text-gray-400">
                    Setelah diselesaikan, stok produk akan diperbarui dan transaksi penyesuaian dicatat otomatis.
                    <strong class="text-yellow-400">Aksi ini tidak bisa dibatalkan.</strong>
                </p>
            </div>
            <form action="{{ route('manager.stock-opname.selesaikan', $opname->id) }}" method="POST">
                @csrf
                <button type="submit"
    ...
    <div class="rounded-2xl border border-green-700/40 bg-green-900/10 p-5">
        <p class="text-green-400 text-sm font-medium">
            ✅ Opname ini sudah diselesaikan. Stok produk telah diperbarui.
        </p>
    </div>
    @endif
