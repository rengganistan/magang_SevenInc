...
<div class="w-full px-4 py-6 sm:px-6 lg:px-8">
    ...
    <div class="flex flex-col lg:flex-row lg:items-center lg:justify-between gap-6 mb-8">
        ...
        <div class="flex gap-3">
            ...
            <a href="{{ route('manager.stock-opname.create') }}"
            ...
            <table class="min-w-full">
                ...
                <tbody class="divide-y divide-gray-700">
                    ...
                    <tr class="hover:bg-gray-700 transition">
                        ...
                        <td class="px-6 py-4 text-white font-medium">{{ $opname->user->name ?? '-' }}</td>
                        <td class="px-6 py-4 text-center text-white">{{ $opname->items->count() }} produk</td>
                        <td class="px-6 py-4 text-center">
                            @if($opname->status === 'Selesai')
                                <span class="px-3 py-1 rounded-full bg-green-600/30 text-green-400 text-xs font-semibold">✅ Selesai</span>
                            @else
                                <span class="px-3 py-1 rounded-full bg-yellow-500/20 text-yellow-400 text-xs font-semibold">📝 Draft</span>
                            @endif
                        </td>
                        ...
                        <td class="px-6 py-4">
                            <div class="flex justify-center gap-2">
                                <a href="{{ route('manager.stock-opname.show', $opname->id) }}"
                                    class="px-3 py-2 rounded-lg bg-blue-600 hover:bg-blue-700 text-white text-sm">
                                    Detail
                                </a>
                                @if($opname->status === 'Draft')
                                <form action="{{ route('manager.stock-opname.destroy', $opname->id) }}"
