...
<div class="w-full px-4 py-6 sm:px-6 lg:px-8">
    ...
    <div class="flex flex-col lg:flex-row lg:items-center lg:justify-between gap-6 mb-8">
        <div>
            ...
            <p class="mt-2 text-gray-400">Daftar transaksi barang masuk. Konfirmasi untuk memperbarui stok.</p>
        </div>
        <div class="flex gap-3">
            ...
            <a href="{{ route('staff.transactions.create', ['type' => 'Masuk']) }}"
                class="px-5 py-3 rounded-xl bg-green-600 hover:bg-green-700 text-white">+ Catat Masuk</a>
        </div>
    </div>
        ...
        <div class="overflow-x-auto">
            <table class="min-w-full">
                ...
                <tbody class="divide-y divide-gray-700">
                    ...
                    <tr class="hover:bg-gray-700/50 transition {{ $tx->status === 'Pending' ? 'bg-yellow-900/5' : '' }}">
                        ...
                        <td class="px-5 py-4 text-center">
                            @if($tx->status === 'Pending')
                                <span class="px-3 py-1 rounded-full bg-yellow-500/20 text-yellow-400 text-xs font-semibold animate-pulse">
                                    ⏳ Pending
                                </span>
                            @else
                        ...
                        <td class="px-5 py-4 text-center">
                            ...
                            <form action="{{ route('staff.transactions.confirm', $tx->id) }}" method="POST">
                                @csrf
                                <button type="submit"
                                    class="px-4 py-2 rounded-lg bg-green-600 hover:bg-green-700 text-white text-xs font-semibold transition">
                                    ✓ Konfirmasi
                                </button>
                            </form>
                            @else
